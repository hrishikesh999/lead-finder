...
from .sources import discover_via_brave_search, discover_via_podcasts
...
def run(trade: str, country: str, limit: int, dry_run: bool) -> None:
    ...
    brave_candidates = discover_via_brave_search(keywords, country_code, settings)
    podcast_candidates = discover_via_podcasts(keywords, country_code)

    # Merge all sources, deduplicating by domain
    seen_domains: set[str] = set()
    raw_candidates: list = []
    for c in yt_candidates + brave_candidates + podcast_candidates:
    ...
    logger.info(
        "Discovered {} total ({} YouTube, {} Brave, {} podcasts)",
        len(raw_candidates), len(yt_candidates), len(brave_candidates), len(podcast_candidates),
    )
